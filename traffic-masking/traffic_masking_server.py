...
import hashlib
import numpy as np
from masking_lib import TrafficProfile, DynamicObfuscator, stream_generator
...
class TrafficPattern:
    ...
    def random_walk(self, base_rate):
        ...
        self.walk_value = max(base_rate * 0.3, min(base_rate * 2, self.walk_value + change))
...
class PacketGenerator:
    ...
    def generate_packet(self, target_size=None):
        ...
        self.sequence += 1
        timestamp = struct.pack('!Q', int(time.time() * 1000000))  # микросекунды
        ...
        data_size = size - 28  # 4 + 8 + 16 = 28 байт заголовка
...
class MaskingTrafficServer:
    ...
    def __init__(self, host='0.0.0.0', port=8888, target_mbps=5, min_mbps=None, max_mbps=None, advanced=False, profile='mixed', header='none', padding='random', mtu=1200, entropy=1.0, stats_interval=5.0):
        self.host = host
        self.port = port
        ...
        try:
            self.profile = TrafficProfile(profile) if isinstance(profile, str) else (profile or TrafficProfile.MIXED)
        except Exception:
    ...
    def start(self):
        ...
        print(f"[*] Traffic masking server started on {self.host}:{self.port}", flush=True)
        if self.min_mbps is not None and self.max_mbps is not None:
            print(f"[*] Floating throughput: {self.min_mbps}-{self.max_mbps} Mbps", flush=True)
        else:
        ...
        if self.advanced:
            ...
            self.generator = stream_generator(
                self.profile,
                target_mbps=self.target_mbps if (self.min_mbps is None or self.max_mbps is None) else None,
                min_mbps=self.min_mbps,
                max_mbps=self.max_mbps,
                obfuscator=self.obfuscator,
                entropy=self.entropy,
            )
            print(f"[*] Advanced mode enabled: profile={self.profile.value}, header={self.header_mode}, padding={self.padding_strategy}, mtu={self.mtu}, entropy={self.entropy}", flush=True)
    ...
    def send_loop(self):
        ...
        while self.running:
            ...
            while bytes_accumulator > 0 and self.clients and packets_sent_this_round < 50:
    ...
    def stats_loop(self):
        ...
        while self.running:
            ...
            if elapsed > 0:
                ...
                print(f"[STATS] Clients: {len(self.clients)} | "
                      f"Rate: {mbps:.2f} Mbps | "
                      f"PPS: {pps:.0f} | "
...
def main():
    ...
    parser.add_argument('--mbps', type=float, default=5, help='Target rate in Mbps (fixed rate if min/max not specified)')
    parser.add_argument('--min-mbps', type=float, default=None, help='Minimum rate in Mbps for floating rate mode')
    parser.add_argument('--max-mbps', type=float, default=None, help='Maximum rate in Mbps for floating rate mode')
    parser.add_argument('--advanced', action='store_true', help='Enable advanced masking (generator/obfuscator)')
    parser.add_argument('--profile', choices=['web','video','voip','file','gaming','mixed'], default='mixed', help='Traffic profile for advanced mode')
    parser.add_argument('--header', choices=['none','rtp','quic'], default='none', help='Pseudo-header type in advanced mode')
    parser.add_argument('--padding', choices=['random','fixed_buckets','progressive','none'], default='random', help='Padding strategy in advanced mode')
